...
/* --- New DDC/CI Brightness Reader --- */
double read_ddc_brightness() {
    char buf[128];
    double current = 0, max = 100;
    FILE *fp = popen("ddcutil getvcp 10 --terse 2>/dev/null", "r");
    if (fp) {
        if (fgets(buf, sizeof(buf), fp)) {
            // Parses "VCP 10 C current max"
            sscanf(buf, "VCP 10 C %lf %lf", &current, &max);
        }
        pclose(fp);
    }
...
void init_sensors(SensorContext *ctx, const AppConfig *cfg) {
    ...
    /* Topology Discovery: Determine thread vs core mapping */
    ...
    char ssd_path[256] = {0};
    // IMPROVEMENT: Removed '-L' to prevent symlink-hopping into RAM sensors.
    // Uses a more constrained search to find the hwmon node for the boot device controller.
    FILE *fp_ssd = popen("DEV=$(lsblk -no PKNAME $(findmnt -n -o SOURCE / | cut -d'[' -f1) 2>/dev/null); "
    "find /sys/block/$DEV/device/hwmon/hwmon*/ -name 'temp1_input' 2>/dev/null | head -n1", "r");

    if (fp_ssd) {
        if (fgets(ssd_path, sizeof(ssd_path), fp_ssd)) ssd_path[strcspn(ssd_path, "\n")] = 0;
        pclose(fp_ssd);
    }

    if (ssd_path[0] != '\0') ctx->fp_ssd_temp = fopen_nobuf(ssd_path);
    if (!ctx->fp_ssd_temp) ctx->fp_ssd_temp = find_and_open_hwmon(cfg->hw_disk, "temp1_input");
    ...
    /* Probe Dynamic Hardware Limits */
    ctx->fp_cpu_crit = find_and_open_hwmon(cfg->hw_cpu, "temp1_crit");
    if (!ctx->fp_cpu_crit) ctx->fp_cpu_crit = find_and_open_hwmon(cfg->hw_cpu, "temp1_max");

    ctx->fp_ssd_crit = find_and_open_hwmon(cfg->hw_disk, "temp1_crit");
    if (!ctx->fp_ssd_crit) ctx->fp_ssd_crit = find_and_open_hwmon(cfg->hw_disk, "temp1_max");
...
SystemVitals read_fast_vitals(SensorContext *ctx, const AppConfig *cfg) {
    ...
    /* Read dynamic safety limits */
    if (ctx->fp_cpu_crit) {
        rewind(ctx->fp_cpu_crit);
        if (fscanf(ctx->fp_cpu_crit, "%lf", &val_buf) == 1) v.cpu_crit = val_buf / 1000.0;
    } else { v.cpu_crit = 80.0; } /* Fallback */

        if (ctx->fp_ssd_crit) {
            rewind(ctx->fp_ssd_crit);
            if (fscanf(ctx->fp_ssd_crit, "%lf", &val_buf) == 1) v.ssd_crit = val_buf / 1000.0;
        } else { v.ssd_crit = 70.0; } /* Fallback */
...
double check_audio_volume(SensorContext *ctx) {
    ...
    /* RE-SYNC: This tells ALSA to update the internal state from the system */
...
void cleanup_sensors(SensorContext *ctx) {
    ...
    /* Clean up CPU pread descriptors */

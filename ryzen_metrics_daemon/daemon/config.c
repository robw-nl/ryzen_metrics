...
#define SET_STR(var, val) do { strncpy(var, val, sizeof(var) - 1); var[sizeof(var) - 1] = '\0'; } while(0)
...
void set_defaults(AppConfig *c) {
    ...
    snprintf(c->path_data, MAX_PATH, "%s/.config/ryzen_metrics_daemon_release/stats.dat", home);
...
AppConfig load_config(const char *path) {
    AppConfig c;
    set_defaults(&c);      /* Initializes all fields with safe values */
    discover_hardware(&c); /* Probes paths, can be overridden by the file  */
    ...
    if (!f) {
        ...
        /* Safety: Stop if the source of truth is missing */
    }
